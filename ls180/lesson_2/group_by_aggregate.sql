...
-- List all genres for which there is a move in the `films` table
SELECT DISTINCT genre
  FROM films;

-- List all genres for which there is a move in the `films` table without using DISTINCT
SELECT genre
  FROM films
 GROUP BY genre;

-- Determine the average duration across all films, rounded to the nearest minute
SELECT ROUND(AVG(duration))
  FROM films;

-- Determine the average duration for each genre, rounded to the nearest minute
SELECT genre, ROUND(AVG(duration)) AS average_duration
  FROM films
 GROUP BY genre;

-- Determine the average duration of movies for each decade, rounded to the nearest minute and listed in chronological order
...
-- Find a films whose director has the first name 'John'
SELECT *
  FROM films
 WHERE director LIKE 'John %';

-- Return a count of films for each genre, ordered from most to least films
SELECT genre, COUNT(id)
  FROM films
 GROUP BY genre
 ORDER BY count DESC;

-- Return an aggregate of films in the same decade and genre, ordered by decade then genre
SELECT (year / 10) * 10 AS decade,
       genre,
       STRING_AGG(title, ', ')
  FROM films
...
-- Return the total sum of duration of all films for each genre, starting with the shortest duration then genre
