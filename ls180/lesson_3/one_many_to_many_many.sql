...
-- List all tables in the database and display their schemas
\dt
\d directors
\d films

-- Create a join table that will allow a film to have multiple directors, and directors to have multiple films
-- Include an `id` column, and foreign key constraints on other column
...
-- Insert data into the new join table to represent the existing one-to-many relationships
...
-- Remove unwanted columns from `films`
ALTER TABLE films
DROP COLUMN director_id;

\d films

-- Return a list of film titles (`title`) and their respective director (`name`) in alphabetical order based on title, then director
...
-- Determine how many films each director has directed, sorted by most number of films and then name in alphabetical order
SELECT d.name, COUNT(df.film_id) AS films
  FROM directors AS d
  JOIN directors_films AS df
    ON d.id = df.director_id
 GROUP BY d.name
...
DROP TABLE directors, films, directors_films;
