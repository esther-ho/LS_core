...
-- Display all tables in database and their schemas
\dt
\d calls
\d contacts

-- Add data to the `calls` table
...
-- Return the call times, durations, and first name for all calls NOT made to William Swift
...
-- Insert data to the `contacts` and `calls` tables
...
-- Add a constraint to `contacts` to prevent a duplicate value from being added in the column `number`
ALTER TABLE contacts
  ADD UNIQUE (number);

\d contacts

-- Prove that an error is raised if a duplicate number is used for a new contact
INSERT INTO contacts (first_name, last_name, number)
VALUES ('Merve', 'Elk', '6343511126');

-- Delete all tables in database
DROP TABLE calls, contacts;
